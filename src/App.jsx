      ...
      <div className="app">
        ...
        {tab === 'home' && <>
          <section className="hero-card v3-hero animate-in">
            ...
            <div className="health-badge"><span>FINANCIAL HEALTH</span><strong>{financialHealth}</strong><small>/100 · {healthLabel}</small></div>
          </section>
          ...
          <section className="card smart-advice-card animate-in"><div className="advice-icon">✦</div><div><span className="eyebrow">{t.smartAdvice}</span><h3>{t.healthyPlan}</h3><p>{smartAdvice}</p><div className="advice-metrics"><div><span>{t.safeToSpend}</span><b>{money(safeDailySpend)}</b><small>/ zi</small></div><div><span>{t.projectedSavings}</span><b>{money(targetMonthlySavings)}</b><small>/ lună</small></div></div></div></section>
        ...
        {tab === 'statistics' && <>
          ...
          <section className="health-card card"><div className="health-top"><div><span className="eyebrow">FINANCIAL HEALTH</span><h2>{financialHealth}/100</h2><p>{healthLabel}</p></div><div className="health-ring" style={{'--p':`${financialHealth*3.6}deg`}}><span>{financialHealth}</span></div></div><div className="health-track"><div className="health-fill" style={{width:`${financialHealth}%`}}/></div></section>
          <section className="card"><div className="section-title"><div><span className="eyebrow">CHELTUIELI FIXE</span><h2>{ui('Lunar de bază','Monthly essentials','Ежемесячные базовые')}</h2></div><strong>{money(profileFixedCosts)}</strong></div><div className="fixed-cost-list">{[[t.rent,onboardingData.rent,'🏠'],[t.bills,onboardingData.bills,'💡'],[t.food,onboardingData.food,'🍔'],[t.transport,onboardingData.transport,'🚗'],[t.subscriptions,onboardingData.subscriptions,'📱'],[t.otherCosts,onboardingData.other,'＋']].map(([label,value,icon])=><div className="fixed-cost" key={label}><span>{icon} {label}</span><b>{money(Number(value||0))}</b></div>)}</div></section>
          <section className="card chart-card"><div className="section-title"><div><span className="eyebrow">LAST 7 DAYS</span><h2>{ui('Trend cheltuieli','Spending trend','Динамика расходов')}</h2></div><strong>{money(weekTotal)}</strong></div><div className="spark-chart"><div className="spark-grid"><i/><i/><i/></div><svg viewBox="0 0 350 130" preserveAspectRatio="none" className="spark-svg"><polyline points={dailyTrend.map((d,i)=>`${i*58.3+3},${125-Math.min(105,(d.value/Math.max(...dailyTrend.map(x=>x.value),1))*105)}`).join(' ')} fill="none" pathLength="1" className="spark-line"/>{dailyTrend.map((d,i)=><circle key={d.key} cx={i*58.3+3} cy={125-Math.min(105,(d.value/Math.max(...dailyTrend.map(x=>x.value),1))*105)} r="3.5" className="spark-dot"/>)}</svg></div><div className="trend-labels">{dailyTrend.map(d=><span key={d.key}>{d.label}</span>)}</div></section>
          ...
          <section className="card"><div className="section-title"><div><span className="eyebrow">PAYMENT MIX</span><h2>Cash vs Card</h2></div><strong>{money(cashSpend+cardSpend)}</strong></div><div className="donut-wrap"><div className="donut" style={{'--cash':`${cashSpend/Math.max(cashSpend+cardSpend,1)*100}%`}}><span>{Math.round(cashSpend/Math.max(cashSpend+cardSpend,1)*100)}%</span></div><div className="donut-legend"><div>💵 Cash <b>{money(cashSpend)}</b></div><div>💳 Card <b>{money(cardSpend)}</b></div></div></div></section>
