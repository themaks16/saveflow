    ...
    <div className="app-shell saveflow-v3">
      <div className="app">
        ...
        {tab === 'statistics' && <>
          ...
          <section className="card fixed-cost-card"><div className="section-title"><div><span className="eyebrow">CHELTUIELI FIXE · {new Intl.DateTimeFormat(language==='en'?'en-US':language==='ru'?'ru-RU':'ro-MD',{month:'long'}).format(new Date())}</span><h2>{ui('Lunar de bază','Monthly essentials','Ежемесячные базовые')}</h2></div><div className="fixed-cost-actions"><strong>{money(profileFixedCosts)}</strong><button className="icon-button" onClick={()=>setModal('monthlyPlan')} aria-label={ui('Editează cheltuielile fixe','Edit fixed costs','Изменить постоянные расходы')}>✎</button></div></div><p className="fixed-cost-note">{ui('Actualizează-le în fiecare lună. Cheltuielile reale rămân urmărite în tranzacții.','Update these each month. Actual spending stays tracked in transactions.','Обновляйте каждый месяц. Фактические траты остаются в операциях.')}</p><div className="fixed-cost-list">{[[t.rent,onboardingData.rent,'🏠'],[t.bills,onboardingData.bills,'💡'],[t.food,onboardingData.food,'🍔'],[t.transport,onboardingData.transport,'🚗'],[t.subscriptions,onboardingData.subscriptions,'📱'],[t.otherCosts,onboardingData.other,'＋']].map(([label,value,icon])=><div className="fixed-cost" key={label}><span>{icon} {label}</span><b>{money(Number(value||0))}</b></div>)}</div><button className="text-action" onClick={()=>setModal('monthlyPlan')}>{ui('Editează planul acestei luni →','Edit this month’s plan →','Изменить план на месяц →')}</button></section>
