...
export const daysAgo = (days, date = new Date()) => {
...
export const isWithinDays = (value, days, now = new Date()) => {
  ...
  const limit = Number(days || 0) * 86400000
  return !Number.isNaN(d.getTime()) && d.getTime() <= now.getTime() && now.getTime() - d.getTime() <= limit
}
...
export const calculateHealth = ({ income = 0, expenses = 0, savings = 0, weeklySpent = 0, weeklyBudget = 0, emergencyMonths = 0 }) => {
  ...
  const budgetScore = weeklyBudget > 0 ? Math.max(0, Math.min(1, 1 - Number(weeklySpent) / Number(weeklyBudget))) : 0.5
  const emergencyScore = Math.min(1, Math.max(0, Number(emergencyMonths) / 3))
  const score = 30 * Math.max(0, 1 - spendingRate) + 25 * savingRate + 20 * budgetScore + 15 * emergencyScore + 10 * (savingRate > 0 ? 1 : 0)
...
export const calculateEmergencyMonths = ({ savings = 0, monthlyEssentialCosts = 0 }) => {
  const costs = Number(monthlyEssentialCosts) || 0
  return costs > 0 ? Math.max(0, Number(savings) || 0) / costs : 0
}

export const calculateSpendingPace = ({ spent = 0, daysElapsed = 1, monthDays = 30, budget = 0 }) => {
  const elapsed = Math.max(1, Number(daysElapsed) || 1)
  const projected = Number(spent || 0) / elapsed * Number(monthDays || 30)
  return { projected, ratio: budget > 0 ? projected / budget : 0 }
}

export const formatMoney = (amount, currency = 'MDL', locale = 'ro-MD') => new Intl.NumberFormat(locale, {
