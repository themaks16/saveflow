...
export const isWithinDays = (value, days, now = new Date()) => {
  ...
  return !Number.isNaN(d.getTime()) && now.getTime() - d.getTime() <= Number(days) * 86400000 && d <= now
}
...
export const calculateAffordability = ({ balance = 0, purchase = 0, safeDailySpend = 0, safetyDays = 3 }) => {
  ...
  const safeLimit = Math.max(0, Number(safeDailySpend) || 0) * Number(safetyDays || 3)
  return {
    purchase: p,
    after: b - p,
    safeLimit,
    affordable: p > 0 && p <= b && p <= safeLimit,
  }
}
...
export const calculateMonthlyMetrics = ({ transactions = [], savings = [], month = localMonthKey() }) => {
  ...
  return { income, expenses, saved, balanceChange, savingsRate, spendingRate }
}
